    ...
    <div style={{ padding:'0 28px 52px' }}>
      ...
      {showForm && (
        <div style={{ background:'var(--bg2)', border:'1px solid rgba(45,127,249,0.2)', borderRadius:14, padding:'18px 20px', marginBottom:20 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr auto 1fr 1fr', gap:12, marginBottom:12 }}>
            {[['Ticker','ticker','AAPL'],['Qty','qty','100'],['Price ($)','price','150.00']].map(([l,k,p])=>(
              <div key={k}><div style={{ fontSize:10.5, color:'var(--text2)', marginBottom:5 }}>{l}</div><input className="qd-input" placeholder={p} value={(form as any)[k]} onChange={e=>setForm(x=>({...x,[k]:e.target.value}))}/></div>
            ))}
            <div>
