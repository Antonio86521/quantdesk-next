    ...
    sectionHTML.push(`
      <section>
        ...
        <div class="metrics-grid">
          ...
          <div class="metric"><div class="metric-label">Ann. Volatility</div><div class="metric-value">${fmtPct(s.annVol)}</div></div>
          ...
          <div class="metric"><div class="metric-label">Beta vs ${s.benchmark}</div><div class="metric-value">${fmtNum(s.beta)}</div></div>
          <div class="metric"><div class="metric-label">Unrealised P&L</div><div class="metric-value ${s.unrealisedPnl>=0?'pos':'neg'}">${fmtDollar(s.unrealisedPnl)}</div></div>
        </div>
        <h3>Holdings</h3>
        <table>
          ...
          <tbody>
            ${h.map((hh:any)=>`<tr>
              <td><strong>${hh.ticker}</strong></td>
              <td>${hh.shares}</td>
              <td>$${hh.buyPrice.toFixed(2)}</td>
              <td>$${hh.currentPrice.toFixed(2)}</td>
              <td>${fmtDollar(hh.marketValue)}</td>
              <td class="${hh.unrealisedPnl>=0?'pos':'neg'}">${hh.unrealisedPnl>=0?'+':''}${fmtDollar(hh.unrealisedPnl)}</td>
              <td>${(hh.weight*100).toFixed(1)}%</td>
              <td class="${hh.periodReturn>=0?'pos':'neg'}">${fmtPlus(hh.periodReturn)}</td>
            </tr>`).join('')}
          </tbody>
        </table>
      </section>`)
    ...
    sectionHTML.push(`
      <section>
        ...
        <div class="metrics-grid">
          ...
          <div class="metric"><div class="metric-label">Parametric VaR</div><div class="metric-value neg">${fmtPct(s.paramVar95)}</div></div>
          <div class="metric"><div class="metric-label">Sortino Ratio</div><div class="metric-value ${s.sortino>1?'pos':''}">${fmtNum(s.sortino)}</div></div>
          <div class="metric"><div class="metric-label">Calmar Ratio</div><div class="metric-value ${s.calmar>1?'pos':''}">${fmtNum(s.calmar)}</div></div>
          <div class="metric"><div class="metric-label">Info Ratio</div><div class="metric-value ${s.infoRatio>0?'pos':'neg'}">${fmtNum(s.infoRatio)}</div></div>
          <div class="metric"><div class="metric-label">Tracking Error</div><div class="metric-value">${fmtPct(s.trackingError)}</div></div>
          <div class="metric"><div class="metric-label">R² vs Benchmark</div><div class="metric-value">${fmtNum(s.r2,3)}</div></div>
        </div>
        <h3>Additional Risk Metrics</h3>
        <table>
          <thead><tr><th>Metric</th><th>Value</th><th>Interpretation</th></tr></thead>
          <tbody>
            <tr><td>Skewness</td><td>${fmtNum(s.skewness,3)}</td><td>${s.skewness>0?'Positive tail risk':'Negative tail risk'}</td></tr>
            <tr><td>Excess Kurtosis</td><td>${fmtNum(s.kurtosis,3)}</td><td>${s.kurtosis>3?'Fat tails present':'Normal tail behaviour'}</td></tr>
            <tr><td>Omega Ratio</td><td>${fmtNum(s.omega)}</td><td>${s.omega>1?'Gains exceed losses':'Losses exceed gains'}</td></tr>
            <tr><td>Gain/Pain Ratio</td><td>${fmtNum(s.gainToPain)}</td><td>${s.gainToPain>1?'Favourable':'Unfavourable'}</td></tr>
          </tbody>
        </table>
      </section>`)
  }
        ...
        <table>
          ...
          <tbody>
            ${market.map(m=>`<tr>
              ...
              <td>${m.value}</td>
              <td class="${m.up?'pos':'neg'}">${m.chgStr}</td>
              <td>${m.up?'↑ Rising':'↓ Falling'}</td>
            </tr>`).join('')}
          </tbody>
        </table>
      </section>`)
  ...
  if (sections.includes('technical')) {
    sectionHTML.push(`
      <section>
        ...
        <p>Technical analysis was requested for: <strong>${tickers}</strong> over the <strong>${period}</strong> period.</p>
        <p>Key indicators monitored: RSI (14), MACD, Bollinger Bands, SMA 20/50.</p>
        <table>
          ...
          <tbody>
            ...
            <tr><td>Bollinger Bands</td><td>Mid-band</td><td>Neutral — no extreme</td></tr>
          </tbody>
        </table>
      </section>`)
  }
...
<title>QuantDesk Pro — Portfolio Report</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Segoe UI', system-ui, sans-serif; background: #f8fafc; color: #1e293b; padding: 40px; font-size: 13px; }
  .wrapper { max-width: 1000px; margin: 0 auto; background: #fff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 40px; box-shadow: 0 4px 24px rgba(0,0,0,0.06); }
  ...
  .logo-mark { width: 40px; height: 40px; border-radius: 10px; background: linear-gradient(135deg,#2d7ff9,#7c5cfc); display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 800; font-size: 15px; }
  .logo-name { font-size: 18px; font-weight: 700; color: #0f172a; }
  .logo-sub  { font-size: 11px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.8px; margin-top: 2px; }
  .report-meta { text-align: right; }
  .report-title { font-size: 14px; font-weight: 600; color: #0f172a; }
  .report-date  { font-size: 11px; color: #64748b; margin-top: 3px; }
  section { margin-bottom: 36px; }
  h2 { font-size: 16px; font-weight: 700; color: #0f172a; border-left: 3px solid #2d7ff9; padding-left: 10px; margin-bottom: 16px; }
  h3 { font-size: 13px; font-weight: 600; color: #334155; margin: 20px 0 10px; }
  ...
  .metric-label { font-size: 10px; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px; }
  .metric-value { font-size: 18px; font-weight: 300; color: #0f172a; font-family: 'Courier New', monospace; }
  table { width: 100%; border-collapse: collapse; margin-top: 8px; }
  th { text-align: left; padding: 8px 12px; font-size: 10px; color: #64748b; font-weight: 700; letter-spacing: 0.6px; text-transform: uppercase; border-bottom: 1px solid #e2e8f0; background: #f8fafc; }
  td { padding: 10px 12px; border-bottom: 1px solid #f1f5f9; font-size: 12.5px; }
  ...
  @media print {
    ...
    .wrapper { border: none; box-shadow: none; padding: 20px; }
  }
</style>
</head>
<body>
<div class="wrapper">
  <div class="header">
    <div class="logo">
      ...
      <div><div class="logo-name">QuantDesk Pro</div><div class="logo-sub">Portfolio Intelligence Platform</div></div>
    </div>
    <div class="report-meta">
      <div class="report-title">Portfolio Analysis Report</div>
      <div class="report-date">Generated: ${now}</div>
      <div class="report-date">Period: ${period} · Benchmark: ${s?.benchmark||'SPY'}</div>
    </div>
  </div>
  ${sectionHTML.join('\n')}
  <div class="footer">
    <div>© 2026 QuantDesk Pro — Educational use only</div>
    <div>Not investment advice · Data via yfinance</div>
  </div>
</div>
</body>
</html>`
...
export default function ReportsPage() {
  ...
  const downloadHTML = () => {
    ...
    a.href     = url
    a.download = `quantdesk-report-${new Date().toISOString().slice(0,10)}.html`
    a.click()
  ...
  return (
    <div style={{ padding:'0 28px 52px' }}>
      <div style={{ margin:'24px 0 20px' }}>
        <h1 style={{ fontSize:24, fontWeight:700, letterSpacing:'-0.5px', marginBottom:5 }}>Report Generator</h1>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:12, marginBottom:24 }}>
        {REPORT_TYPES.map(r => (
          <div key={r.id} onClick={()=>toggle(r.id)} style={{
            background: selected.includes(r.id) ? r.bg : 'var(--bg2)',
            border:`1px solid ${selected.includes(r.id) ? r.color+'44' : 'var(--b1)'}`,
            ...
            <div style={{ display:'flex', alignItems:'center', gap:14 }}>
              ...
              <div style={{
                ...
                border:`2px solid ${selected.includes(r.id) ? r.color : 'var(--b2)'}`,
                background: selected.includes(r.id) ? r.color : 'transparent',
      ...
      <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
        <button onClick={generate} disabled={!selected.length || loading} style={{
          padding:'10px 24px', borderRadius:8, cursor: selected.length&&!loading?'pointer':'not-allowed',
          background: selected.length&&!loading ? 'linear-gradient(135deg,#2d7ff9,#1a6de0)' : 'var(--bg4)',
          border:'1px solid rgba(45,127,249,0.4)', color:'#fff', fontSize:13, fontWeight:600,
        ...
        {generated && (<>
          ...
          <button onClick={printPDF} style={{ padding:'10px 20px', borderRadius:8, background:'var(--bg3)', border:'1px solid var(--b2)', color:'var(--text)', fontSize:13, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:8 }}>
            🖨 Print / Save PDF
          </button>
          <button onClick={preview} style={{ padding:'10px 20px', borderRadius:8, background:'var(--bg3)', border:'1px solid var(--b2)', color:'var(--text)', fontSize:13, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:8 }}>
            👁 Preview
          </button>
        </>)}
      </div>
        ...
        <div style={{ marginTop:16, background:'rgba(13,203,125,0.08)', border:'1px solid rgba(13,203,125,0.2)', borderRadius:12, padding:'14px 18px', display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:12.5, color:'var(--text2)' }}>
              Sections: {selected.map(s=>REPORT_TYPES.find(r=>r.id===s)?.name).join(' · ')}
            </div>
            <div style={{ fontSize:12, color:'var(--text3)', marginTop:3 }}>
              Click <strong>Download HTML</strong> to save, or <strong>Print / Save PDF</strong> to use your browser's print dialog → Save as PDF
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ marginTop:24, background:'var(--bg2)', border:'1px solid var(--b1)', borderRadius:14, padding:'16px 20px' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {['1. Select your report sections above', '2. Click Generate Report (fetches live data from your Python backend)', '3. Click Print / Save PDF — your browser print dialog opens', '4. In the print dialog, choose "Save as PDF" as the destination', '5. Click Save — your professional report is downloaded'].map((step, i) => (
