  ...
  return (
    <div style={{ padding:'0 28px 52px' }}>
      <div style={{ margin:'24px 0 20px', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div><h1 style={{ fontSize:24, fontWeight:700, letterSpacing:'-0.5px', marginBottom:5 }}>Market Overview</h1><div style={{ fontSize:13, color:'var(--text2)' }}>Live cross-asset snapshot · Sectors · Major indices</div></div>
        ...
        {loading ? [...Array(8)].map((_,i)=><div key={i} style={{ height:90, borderRadius:12 }} className="skeleton"/>) :
          data.map((m,i) => (
            ...
            >
              ...
              <div style={{ fontFamily:'var(--fm)', fontSize:21, fontWeight:300, marginBottom:7, letterSpacing:'-0.5px' }}>{m.value}</div>
