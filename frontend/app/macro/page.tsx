  ...
  return (
    <div style={{ padding:'0 28px 52px' }}>
      <div style={{ margin:'24px 0 20px', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div><h1 style={{ fontSize:24, fontWeight:700, letterSpacing:'-0.5px', marginBottom:5 }}>Macro Dashboard</h1><div style={{ fontSize:13, color:'var(--text2)' }}>Rates · FX · Commodities · Volatility · Equity Indices</div></div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:20 }}>
        {snapshots.map((s,i) => (
          ...
          >
            ...
            <div style={{ fontFamily:'var(--fm)', fontSize:20, fontWeight:300, marginBottom:7 }}>{s.value}</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'160px 1fr', gap:16 }}>
        <div>
          {MACRO_ASSETS.map(a => (
            <button key={a.ticker} onClick={()=>setSelected(a)} style={{ display:'block', width:'100%', padding:'9px 12px', borderRadius:8, marginBottom:4, border:`1px solid ${selected.ticker===a.ticker?'rgba(45,127,249,0.3)':'transparent'}`, background:selected.ticker===a.ticker?'var(--accent3)':'transparent', color:selected.ticker===a.ticker?'var(--accent2)':'var(--text2)', fontSize:12, fontWeight:selected.ticker===a.ticker?600:400, cursor:'pointer', textAlign:'left', transition:'all 0.14s' }}>
              {a.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ background:'var(--bg2)', border:'1px solid var(--b1)', borderRadius:14, padding:'16px 20px' }}>
          ...
          ) : <div style={{ height:280, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text3)' }}>Loading chart data...</div>}
        </div>
      </div>
    </div>
  )
