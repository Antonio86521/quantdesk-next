  ...
  return (
    <div style={{ padding: isMobile ? '0 14px 80px' : '0 28px 52px' }}>
      <div style={{ margin:'24px 0 20px' }}>
        <h1 style={{ fontSize: isMobile?20:24, fontWeight:700, letterSpacing:'-0.5px', marginBottom:5 }}>Volatility Surface</h1>
      ...
      <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '200px 1fr', gap:20, marginBottom:20 }}>
        ...
        <div style={{ background:'var(--bg2)', border:'1px solid var(--b1)', borderRadius:14, padding:'18px 20px' }}>
          ...
          {[
            ['Base Volatility', baseVol, setBaseVol, 0.05, 1.0, 0.01],
            ['Spot Price',      spot,    setSpot,    50,   500, 1   ],
          ].map(([l,v,s,mn,mx,step]:any)=>(
            <div key={l as string} style={{ marginBottom:16 }}>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:10.5, color:'var(--text2)', marginBottom:6 }}>
                <span>{l}</span>
                <span style={{ fontFamily:'var(--fm)', color:'var(--accent2)' }}>{Number(v).toFixed(2)}</span>
              </div>
      ...
      <div style={{ background:'var(--bg2)', border:'1px solid var(--b1)', borderRadius:14, overflow:'hidden' }}>
        ...
        <div style={{ overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
          <table className="qd-table" style={{ minWidth: isMobile ? 480 : 'auto' }}>
            <thead>
              <tr>
                <th>Strike / Spot</th>
                {EXPIRIES.map(T=><th key={T}>{(T*12).toFixed(0)}M</th>)}
              </tr>
            </thead>
            <tbody>
              {STRIKES.map((k,i)=>(
