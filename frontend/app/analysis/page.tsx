...
import { AreaChart, Area, LineChart, Line, BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, ReferenceLine, Cell, PieChart, Pie, Legend } from 'recharts'
import { TrendingUp, RefreshCw, Download, ChevronDown, ChevronUp, BarChart3, AlertTriangle, BookOpen } from 'lucide-react'

type SavedEntry = {
  id: number; name: string; date: string; tickers: string; shares: string; buyPrices: string; period: string; benchmark: string; note: string; tag: string
}
...
function AnalysisPanel({ entry, onClose }: { entry: SavedEntry; onClose: () => void }) {
  ...
  useEffect(() => {
    api.portfolio.analytics({
      tickers: entry.tickers, shares: entry.shares, buyPrices: entry.buyPrices,
      period: entry.period, benchmark: entry.benchmark,
    ...
    <div style={{ marginTop:16, background:'var(--bg3)', border:'1px solid var(--b2)', borderRadius:14, overflow:'hidden' }}>
      ...
      <div style={{ padding:'14px 20px', borderBottom:'1px solid var(--b1)', display:'flex', alignItems:'center', justifyContent:'space-between', background:'var(--bg4)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          {loading && <span style={{ fontSize:11, color:'var(--text3)', display:'flex', alignItems:'center', gap:5 }}><RefreshCw size={10} style={{ animation:'spin 1s linear infinite' }} /> Loading live data...</span>}
      ...
      {data && s && (
        <div style={{ padding:'16px 20px' }}>
          {/* Tabs */}
          <div style={{ display:'flex', gap:3, marginBottom:16, background:'var(--bg5)', border:'1px solid var(--b1)', borderRadius:8, padding:3, width:'fit-content' }}>
            {(['overview','performance','risk','holdings'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} style={{ padding:'6px 16px', borderRadius:6, border:'none', cursor:'pointer', background:tab===t?'var(--bg2)':'transparent', color:tab===t?'var(--text)':'var(--text2)', fontSize:11.5, fontWeight:tab===t?600:400, transition:'all 0.14s', textTransform:'capitalize', boxShadow:tab===t?'0 2px 6px rgba(0,0,0,0.25)':'none' }}>{t}</button>
            ))}
          </div>
          ...
          {tab==='overview' && (
            <>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
                <div>
                  {[
                    ['Ann. Return',      fmtPctPlus(s.annReturn),    s.annReturn>=0],
                    ['Ann. Volatility',  fmtPct(s.annVol),           null],
                    ['Sortino Ratio',    fmtNum(s.sortino),           s.sortino>1],
                    ['Calmar Ratio',     fmtNum(s.calmar),            s.calmar>1],
                    ['Beta',             fmtNum(s.beta),              null],
                    ['Alpha (Ann.)',     fmtPctPlus(s.alpha),         s.alpha>0],
                  ].map(([l,v,up]:any, i) => (
                ...
                <div>
                  {[
                    ['VaR 95%',           fmtPct(s.histVar95),       false],
                    ['CVaR 95%',          fmtPct(s.histCVar95),       false],
                    ['Omega Ratio',       fmtNum(s.omega),            s.omega>1],
                    ['R² vs Benchmark',  fmtNum(s.r2,3),             null],
                    ['Tracking Error',   fmtPct(s.trackingError),    null],
                    ['Info Ratio',       fmtNum(s.infoRatio),        s.infoRatio>0],
                  ].map(([l,v,up]:any, i) => (
                    <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'9px 0', borderBottom: i<5?'1px solid rgba(255,255,255,0.04)':'none' }}>
                      <span style={{ fontSize:12.5, color:'var(--text2)' }}>{l}</span>
                      <span style={{ fontFamily:'var(--fm)', fontSize:12.5, color: up===true?'var(--green)':up===false?'var(--red)':'var(--text)' }}>{v}</span>
              ...
              <div style={{ background:'var(--bg2)', border:'1px solid var(--b1)', borderRadius:12, padding:'14px 18px' }}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={data.charts.performance}>
                    <defs>
                      <linearGradient id={`ag${entry.id}`} x1="0" y1="0" x2="0" y2="1">
            ...
            <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:14 }}>
              <div style={{ background:'var(--bg2)', border:'1px solid var(--b1)', borderRadius:12, overflow:'hidden' }}>
                <table className="qd-table">
                  <thead><tr><th>Ticker</th><th>Weight</th><th>Cost</th><th>P&L</th><th>Return</th></tr></thead>
                  <tbody>
                      ...
                      <tr key={i}>
                        ...
                        <td>
                          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                            <div style={{ height:3, width:50, background:'var(--bg4)', borderRadius:2 }}>
                              <div style={{ height:3, width:`${h.weight*100}%`, background:COLORS[i%COLORS.length], borderRadius:2 }} />
                            </div>
                            <span className="mono" style={{ fontSize:11 }}>{(h.weight*100).toFixed(1)}%</span>
                          </div>
                        </td>
                        <td className="mono">{fmtDollar(h.cost)}</td>
          ...
          {/* Footer actions */}
          <div style={{ display:'flex', gap:8, marginTop:16, paddingTop:14, borderTop:'1px solid var(--b1)' }}>
  ...
  return (
    <div style={{ padding:'0 28px 52px' }}>
      <div style={{ margin:'24px 0 20px', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
          <h1 style={{ fontSize:24, fontWeight:700, letterSpacing:'-0.5px', marginBottom:5 }}>Saved Analysis</h1>
          <div style={{ fontSize:13, color:'var(--text2)' }}>Saved portfolio snapshots · Live data reload · Benchmark diagnostics · Export</div>
        </div>
      </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {filtered.map(entry => (
          <div key={entry.id}>
            {/* Entry card */}
            <div style={{
              background:'var(--bg2)', border:`1px solid ${open===entry.id?'rgba(45,127,249,0.25)':'var(--b1)'}`,
              ...
              <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:16 }}>
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:12, color:'var(--text3)', marginBottom:8 }}>
                    {entry.date} · {entry.period} · Benchmark: {entry.benchmark}
                  </div>
                ...
                <div style={{ display:'flex', gap:8, flexShrink:0, alignItems:'flex-start' }}>
                  ...
                  <button onClick={() => setOpen(open===entry.id?null:entry.id)} style={{
                    display:'flex', alignItems:'center', gap:6, padding:'7px 14px', borderRadius:8,
                    background: open===entry.id?'rgba(45,127,249,0.1)':'var(--bg3)',
                    border:`1px solid ${open===entry.id?'rgba(45,127,249,0.25)':'var(--b1)'}`,
                    color: open===entry.id?'var(--accent2)':'var(--text2)', fontSize:12, fontWeight:600, cursor:'pointer', transition:'all 0.14s',
                  }}>
            ...
            {open === entry.id && <AnalysisPanel entry={entry} onClose={() => setOpen(null)} />}
          </div>
        ))}
      </div>
